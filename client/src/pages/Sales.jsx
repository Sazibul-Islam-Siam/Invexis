    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Record and track sales transactions</p>
        </div>
        ...
        <div className="card border border-emerald-200">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center">
              <HiOutlineCash className="text-2xl text-emerald-600" />
            </div>
            <div>
        ...
        <div className="card border border-blue-200">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center">
              <HiOutlineCube className="text-2xl text-blue-600" />
            </div>
            <div>
        ...
        <div className="card border border-purple-200">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-purple-50 rounded-2xl flex items-center justify-center">
              <HiOutlineReceiptTax className="text-2xl text-purple-600" />
            </div>
            <div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-medium text-gray-600">No sales recorded</h3>
            <p className="text-gray-400 mt-1">Record your first sale to get started</p>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-center py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Items</th>
                    ...
                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Sold By</th>
                  ...
                  {sales.map((sale) => (
                    <tr key={sale._id} className="hover:bg-gray-50 transition-colors">
                      <td className="py-3.5 px-4">
                        <code className="text-sm font-semibold text-primary-600 bg-primary-50 px-2.5 py-1 rounded-lg">
                          {sale.invoiceNo}
                        </code>
                      </td>
                      <td className="py-3.5 px-4">
                        <div className="space-y-1">
                          {sale.items?.map((item, i) => (
                            <div key={i} className="flex items-center gap-2 text-sm">
                              <span className="text-gray-900">{item.product?.name || 'Deleted Product'}</span>
                      ...
                      <td className="py-3.5 px-4 text-gray-500 text-sm">{sale.soldBy?.name || '—'}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white border border-gray-300 rounded-2xl p-6 w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-gray-900">Record New Sale</h2>
              <button
                onClick={closeModal}
            ...
            <div className="bg-white border border-gray-300 rounded-xl p-4 mb-4">
              <p className="text-sm font-medium text-gray-600 mb-3">Add Products</p>
            ...
            {cart.length === 0 ? (
              <div className="text-center py-8 border border-dashed border-gray-300 rounded-xl">
                <HiOutlineShoppingCart className="text-3xl text-gray-300 mx-auto mb-2" />
                <p className="text-sm text-gray-400">No items added yet</p>
                <p className="text-xs text-gray-300">Select a product above to begin</p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded transition-all"
                      >
                      ...
                      <input
                        ...
                        className="w-14 bg-gray-100 border border-gray-300 rounded text-center text-sm text-gray-900 py-1 focus:outline-none focus:ring-1 focus:ring-primary-500"
                      />
                      <button
                        onClick={() => updateCartQty(index, item.quantity + 1)}
                        className="p-1 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded transition-all"
                    ...
                    <p className="text-sm font-semibold text-emerald-600 w-24 text-right">
      ...
      {showReceipt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setShowReceipt(null)}></div>
          <div className="relative bg-white border border-gray-300 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="text-center mb-6">
              <div className="w-14 h-14 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-3">
                <HiOutlineReceiptTax className="text-3xl text-emerald-600" />
              </div>
              <h2 className="text-xl font-semibold text-gray-900">Sale Receipt</h2>
              <code className="text-primary-600 text-sm">{showReceipt.invoiceNo}</code>
            </div>

            <div className="bg-white border border-gray-300 rounded-xl p-4 space-y-2 mb-4">
              {showReceipt.items?.map((item, i) => (
                <div key={i} className="flex justify-between text-sm">
                  <span className="text-gray-600">
                    {item.product?.name} × {item.quantity}
                  </span>
                  <span className="text-gray-700">৳{item.totalPrice?.toLocaleString()}</span>
                </div>
              ))}
              ...
              <div className="flex justify-between text-xs pt-1">
                ...
                <span className="text-gray-500">{showReceipt.soldBy?.name}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-gray-400">Date</span>
                <span className="text-gray-500">{new Date(showReceipt.saleDate).toLocaleString()}</span>
              </div>
            </div>
      ...
      {showDetails && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setShowDetails(null)}></div>
          <div className="relative bg-white border border-gray-300 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-xl font-semibold text-gray-900">Sale Details</h2>
                <code className="text-primary-600 text-sm">{showDetails.invoiceNo}</code>
              </div>
              <button
              ...
              {showDetails.items?.map((item, i) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{item.product?.name || 'Deleted Product'}</p>
                      <p className="text-xs text-gray-400">
                        ৳{item.unitPrice?.toLocaleString()} × {item.quantity}
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      {user?.role === 'admin' && item.totalCost > 0 && (
                        <p className="text-xs text-gray-400">Cost: ৳{item.totalCost?.toLocaleString()}</p>
                      )}
                    </div>
                  </div>
                  ...
                  {user?.role === 'admin' && item.batchAllocations?.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-gray-200">
                      <p className="text-xs font-medium text-gray-500 mb-1">Batch Allocations (FIFO)</p>
            ...
            <div className="bg-white border border-gray-300 rounded-xl p-4 space-y-2">
              ...
              {user?.role === 'admin' && (
                <>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">Total Cost (FIFO)</span>
                    <span className="text-gray-600">৳{showDetails.totalCost?.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">Profit</span>
                    <span className={`font-medium ${(showDetails.totalProfit || 0) >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
              ...
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Sold By</span>
                <span className="text-gray-600">{showDetails.soldBy?.name}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Date</span>
                <span className="text-gray-600">
